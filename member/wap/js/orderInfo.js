require(["vue.min", "axios.min", "es6-promise"], function (Vue, axios) {
	...
	var vm3 = new Vue({
		...
		methods: {
			...
			setCookie: function (cname, cvalue, exdays,pathcontent) { //设置cookie
				var d = new Date();
				d.setTime(d.getTime() + (exdays * 24 * 60 * 60 * 1000));
				var expires = "expires=" + d.toUTCString();
				var path = "path="+pathcontent;
				document.cookie = cname + "=" + cvalue + "; " + expires+"; " + path;
			},
			clearCookie: function (name,path) { //清楚cookie
			...
			getGoodsList: function () { //提取商品列表数据
				var _this = this;
				this.$ajax.post('/w/shop/listWebsiteCustomerCart.do', $.param({
					"sortPdid": [1, 0]
				})).then(function (res) {
			...
			submitOrder: function () {                                              //提交订单的函数
				...
				_this.$ajax.post('/w/shop/listWebsiteCustomerCart.do', $.param({
					"sortPdid": [1, 0]
				})).then(function (res) {
					_this.goodsList = [];
					...
					_this.$ajax.post('/w/shop/addWSCstOrder.do', $.param(submitData)).then(function (res) {
						_this.clearCookie('pdid','/member');                                   //删除cookie文件
						var wcoid = res.data.data;
						console.log(wcoid);
						if (_this.payListId == 2) {                                            //网上支付跳转地址
							window.location.href = "payment.html?wcoid=" + wcoid;
						} else {                                                               //货到付款跳转的地址
							window.location.href = "orderdetail.html?wcoid=" + wcoid;
						}
					}).catch(function (error) {
						console.log(error);
					});
