		...
		success: function (data) {
			...
			if(data.result=="SUCCESS"){
                ...
                if(wsctid){
                    ...
                    wapUtil.showTip("此商品最大购买数量为"+ data.errorMsg.pdstc +"，已经超出最大数量！",2);
                }else{
...
shopUtil.loadCartData = function() {
	...
	if($("#web_customer").length > 0) {
			...
			success: function(data) {
				if(data.result == "SUCCESS") {
					...
					if(CartData.length == 0) {
						var car_null = '<div class="cart_null">' +
							'<i class = "icon m_iconfont icon-cart_light"></i>' +
							'<p>购物车还是空的</p>' +
							'<a href = "../index.html"><p>再去逛逛</p></a></div>'
							$("#m_shopcart .bottom").remove();
							$("#m_shopcart .edit").hide();
							$("#m_shopcart .public_navigation").show();
							$("#m_shopcart .content_section").html(car_null);
							console.log("in in ")
					}
				} else {
					webUtil.showTip(data.errorMsg.msg, 1.5);
		...
		var car_nologin = '<div class="cart_null">' +
			'<i class = "icon m_iconfont icon-cart_light"></i>' +
			'<p>购物车还是空的</p>' +
			'<a href = "../index.html"><p>再去逛逛</p></a></div>'
		$("#m_shopcart .bottom").remove();
		$("#m_shopcart .edit").hide();
		$("#m_shopcart .public_navigation").show();
		$("#m_shopcart .content_section").html(car_nologin);
		console.log("in")
	}
	return CartData;
}
...
//设置更多按钮
shopUtil.setNavigation=function(){
    var $dom=$('.public_navigation .public_navigation_layout');
    if($dom.hasClass("open")){
        $dom.removeClass('open');
        $(".public_navigation_bg").hide();
        $(".public_navigation_bg").unbind();
    }else{
        $dom.removeClass('open').addClass('open');
        $(".public_navigation_bg").show();
        $(".public_navigation_bg").on('touchstart',function(){
            $(".public_navigation_layout").removeClass('open');
            $(".public_navigation_bg").hide();
        })
